from hyperppoCNN_model.model import *
import time
...
def default_device():
    """Returns the default device to use for tensors."""
...
class MLP_GHN(nn.Module):
    ...
    def __init__(self,
        ...
        # Initialize default edges and node features on the correct device
        self.default_edges = torch.zeros((50,4), device=device)
        self.default_edges[:,0] = torch.arange(len(self.default_edges[:,0]), device=device)
        self.default_edges[:,1] = torch.arange(len(self.default_edges[:,0]), device=device) + 1
        self.default_edges[:,2] = 1
        self.default_edges = self.default_edges.long()
        self.default_edges = nn.Parameter(self.default_edges, requires_grad=False)

        self.default_node_feat = torch.zeros(50, device=device).long()
    ...
    def forward(self, nets_torch, shape_ind = None, return_embeddings=False, predict_class_layers=True, bn_train=True):
        ...
        # Calculate total number of parameters across all networks
        total_params = sum(len(list(net.parameters())) for net in nets_torch)
        
        # Create graph edges and node features
        for i in range(len(nets_torch)):
            num_params = len(list(nets_torch[i].parameters()))
            # Create edges for this network
            graph_edges = torch.zeros((num_params, 4), dtype=torch.long, device=self.device)
            graph_edges[:, 0] = torch.arange(num_params, device=self.device)  # source nodes
            graph_edges[:, 1] = torch.arange(num_params, device=self.device) + 1  # target nodes
            graph_edges[:, 2] = 1  # edge type
            graph_edges[:, 3] = i  # graph index
            
            # Create node features for this network
            node_feat = torch.zeros(num_params + 1, dtype=torch.long, device=self.device)
            node_feat += i  # Add graph index to node features
    ...
    def _tile_params(self, w, target_shape):
        ...
        # For CNN layers, ensure we maintain the kernel size
        if len(t) == 4:  # CNN layer
            # First slice to match the target shape
            w = w[:min(t[0], s[0]), :min(t[1], s[1]), :min(t[2], s[2]), :min(t[3], s[3])]
            
            # If we need to expand the kernel size
            if t[2] > s[2] or t[3] > s[3]:
                # Create a new tensor with the target kernel size
                new_w = torch.zeros(t, device=w.device)
                # Center the existing kernel in the new tensor
                start_h = (t[2] - s[2]) // 2
                start_w = (t[3] - s[3]) // 2
                new_w[:, :, start_h:start_h + s[2], start_w:start_w + s[3]] = w
                w = new_w
            
            # Tile out_channels if needed
            if t[0] > s[0]:
                n_out = t[0] // s[0] + 1
                w = w.repeat((n_out, 1, 1, 1))[:t[0]]
            
            # Tile in_channels if needed
            if t[1] > s[1]:
                n_in = t[1] // s[1] + 1
        ...
        # For non-CNN layers, use the original logic
...
class GatedGNN(nn.Module):
    ...
    def forward(self, x, edges, node_graph_ind):
        ...
        # Get unique graph indices and their counts
        unique_graphs, n_nodes = torch.unique(node_graph_ind, return_counts=True)
        B = len(unique_graphs)  # batch size
        C = x.shape[1]  # feature dimension

        # Ensure x has the right number of features
        if x.shape[0] != n_nodes.sum():
            # Pad or truncate x to match the number of nodes
            if x.shape[0] > n_nodes.sum():
                x = x[:n_nodes.sum()]
            else:
                # Pad with zeros if needed
                padding = torch.zeros((n_nodes.sum() - x.shape[0], C), device=x.device)
        ...
        for t in range(self.T):
            for order in traversal_orders:  # forward, backward
                ...
                for node in (all_nodes if order else torch.flipud(all_nodes)):
                    ...
                    # Update node hidden states in parallel for a batch of graphs
