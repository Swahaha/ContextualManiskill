...
from contextual_maniskill.agents.create_scaled_urdf import create_scaled_urdf

@register_agent()
class ContextualPanda(ContextualBaseAgent):
    ...
    _scaled_urdf_cache = {}  # Class-level cache of scaled URDFs
    ...
    def __init__(self, *args, **kwargs):
        link_modifications = kwargs.get('link_modifications', {})
        
        if link_modifications:
            original_urdf = self.urdf_path
            
            for link_name, mods in link_modifications.items():
                if 'scale' in mods:
                    scale = mods['scale']
                    # print(f"Creating scaled URDF with {link_name} scaled to {scale}")
                    
                    # Always create a new temporary URDF file
                    scaled_urdf = create_scaled_urdf(
                        original_urdf,
                        link_name, 
                        scale
                    )
                    
                    # Update the URDF path to use the scaled version
                    self.urdf_path = scaled_urdf
                    # print(f"Using temporary scaled URDF: {self.urdf_path}")
                    break  # Only handle one link for now
